...
리엑트는 <span style = "color: yellowgreen;">JS라이브러리</span>이다.<bR>
<Br>
...
React는 기존의 양방향 바인딩 방식의 [MVC패턴](/docs/CS/patterns.html#mvc)에서 발생한 복잡한 상태관리의 문제점으로 인해 등장한 [Flux패턴](/docs/CS/patterns.html#flux)을 사용한다. 
...
1. 마운트(Mount) - 컴포넌트가 최초로 생성되고 실제 DOM에 삽입되는 과정
2. 업데이트(Update) - 컴포넌트의 상태 혹은 props가 변경될때 다시 발생할 수 있다.
