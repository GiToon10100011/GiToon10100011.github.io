...
이때 브라우저에서 폼 객체 API를 제공하여 사용자의 입출력을 받을 수 있도록 해줬었다. 

광클 -> 서버 프로세스가 지속적으로 생김 -> 컴 성능이 너무 느려짐
DDoS 공격에 너무 취약했음. 인젝션또한 그렇고 

이때문에 유효성 검사가 필요해짐 -> 자바스크립트가 탄생하게 된 계기 
최초엔 연산이나 그런 복잡한 작업이 아니라 단순히 제어를 위해 탄생하여, 폼 객체 api를 제공함. 
...
http-server와 같은 웹서버같은 "도구" 들은 배포시에 올라갈 필요가 없으므로 -D로 설치하는것이 좋다. 
