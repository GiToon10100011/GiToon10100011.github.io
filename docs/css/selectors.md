...
- `.`을 사용한다. 클래스 선택자는 <u>특정 클래스를 가진 요소를 선택</u>한다. 다중 클래스를 가진 요소를 선택하기 위해서는 `.`을 사용하여 클래스를 여러개 지정해줄 수 있다. <span style="color: crimson">이때 주의해야할 점은 띄어쓰기이다.</span>
...
- `#`을 사용한다. 아이디 선택자는 <u>특정 id속성이 있는 요소를 선택</u>한다. id속성은 웹 페이지 내부에서 중복되면 안된다는 규정이 있기에 특정 요소를 선택하는데 유용하다.
...
- ` `(공백)을 사용한다. 자손 선택자는 <u>특정 요소의 자손 요소를 선택</u>한다.<span style="color: crimson;"> >는 직계 자식을 선택하는 것이며, 자손 선택자는 모든 자손을 선택</span>한다.
