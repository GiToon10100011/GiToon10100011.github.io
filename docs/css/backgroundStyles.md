...
- `background-repeat` - 배경 이미지 반복, 값은 `repeat, repeat-x, repeat-y, no-repeat` 등이 있다.
- `background-position` - 배경 이미지 위치, 값은 `left, right, center, top, bottom, px, %` 등이 있다.
- `background-size` - 배경 이미지 크기, 값은 `cover, contain, px, %` 등이 있다. `cover`는 <span style="color: yellowgreen;">원본이미지의 가로폭</span>을 기준으로 채우고, `contain`은 <span style="color: yellowgreen;">원본 이미지의 세로폭</span>을 기준으로 채운다.
...
- `radial-gradient` - 방사형 그라데이션, 중심점에서 바깥으로 퍼지는 형태. `radial-gradient(shape size at position, color-stop1, color-stop2, ...)` 형식으로 사용한다. 
  ...
  - position: `at center` (기본값), `at 30% 50%` 등으로 중심점 지정, linear-gradient와 달리 to수식어가 아닌 at 수식어를 사용한다는것을 알 수 있음. 
