...
- `background` - 아래 속성들의 축약형으로 `background: url("image.jpg") no-repeat center center fixed;`와 같은 형식으로 사용한다. 이 때, 축약형은 <mark style="background: #BBFABBA6;">배경 색상, 배경 이미지, 배경 이미지 반복, 배경 이미지 위치, 배경 이미지 크기, 배경 이미지 고정</mark>을 한번에 지정해줄 수 있다.
...
- `background-size` - 배경 크기, 값은 `cover, contain, px, %` 등이 있다. `cover`는 <mark style="background: #BBFABBA6;">원본이미지의 가로폭</mark>을 기준으로 채우고, `contain`은 <mark style="background: #BBFABBA6;">원본 이미지의 세로폭</mark>을 기준으로 채운다. background-size와 같이 이미지 혹은 미디어 태그의 비율을 유지한채로 크기를 조절하고 싶다면, [object-fit](/docs/html/mediaTags.html#object-fit) 속성을 사용하면 된다.
...
- `radial-gradient` - 방사형 그라데이션, 중심점에서 바깥으로 퍼지는 형태. `radial-gradient(shape size at position, color-stop1, color-stop2, ...)` 형식으로 사용한다.
  ...
  - position: `at center` (기본값), `at 30% 50%`
    등으로 중심점 지정, <mark style="background: #BBFABBA6;">linear-gradient와 달리 to수식어가 아닌 at 수식어를 사용</mark>한다는것을 알 수 있음.
